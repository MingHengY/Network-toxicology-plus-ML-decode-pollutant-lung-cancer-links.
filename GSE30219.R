...
##整理下载数据----
id3<-"GSE30219"
gse3<-getGEO(id3,getGPL = F)
gse3 <- gse3[[1]]
summary(exprs(gse3))
exprs3 <- exprs(gse3)
#提取注释和表达信息
annotation3 <- featureData(gse3)
# 读取芯片平台文件txt
platform_file3 <- read.delim("GPL570-55999.txt", header = TRUE, sep = "\t", comment.char = "#")
#查看平台文件列名
colnames(platform_file3)

##数据处理----
# 假设芯片平台文件中有两列，一列是探针ID，一列是基因名
probe_names <- platform_file3$ID
gene_symbol <- platform_file3$Gene.Symbol

platform_file_set=platform_file3[,c(1,11)]
...
save(exprSet, file ="exprSet_gse30219.RData")


#临床信息
patient3<-data.frame(ID=gse3@phenoData@data[["geo_accession"]],OS.time=gse3@phenoData@data[["follow-up time (months):ch1"]],
                     OS=gse3@phenoData@data[["status:ch1"]],Type=gse3@phenoData@data[["title"]])

library(stringr)
library(dplyr)

patient3.1 <- patient3 %>%
  filter(str_detect(Type, fixed("Lung Squamous Cell Carcinoma")))

patient3.1$Type<-NULL

patient3.1$OS<-ifelse(patient3.1$OS == "ALIVE",0,1)
write.table(patient3.1,"GSE30219Patient.txt",sep = "\t",quote = F, row.names = T)


load("exprSet_gse30219.RData")
patient3.1<-read.table("GSE30219Patient.txt",
...
gse30219<-merge(patient3.1,tarexp1,by="ID")
write.table(gse30219,file = "gse30219_geneset.txt",sep = "\t",quote = F, row.names = T)
