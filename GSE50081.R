...
exprs2 <- exprs(gse2)
#提取注释和表达信息
annotation2 <- featureData(gse2)
# 读取芯片平台文件txt
platform_file2 <- read.delim("GPL570-55999.txt", header = TRUE, sep = "\t", comment.char = "#")
#查看平台文件列名
colnames(platform_file2)

##数据处理----
# 假设芯片平台文件中有两列，一列是探针ID，一列是基因名
probe_names <- platform_file2$ID
gene_symbol <- platform_file2$Gene.Symbol

platform_file_set=platform_file2[,c(1,11)]
...
save(exprSet, file ="exprSet_gse50081.RData")
...
patient2<-read.table("GSE50081Patient.txt",
...
gse50081<-merge(patient2,tarexp1,by="ID")
write.table(gse50081,file = "gse50081_geneset.txt",sep = "\t",quote = F, row.names = T)
