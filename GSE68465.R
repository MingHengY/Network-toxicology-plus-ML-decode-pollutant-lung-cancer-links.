...
exprs1 <- exprs(gse1)
#提取注释和表达信息
annotation1 <- featureData(gse1)
# 读取芯片平台文件txt
platform_file1 <- read.delim("GPL96-57554.txt", header = TRUE, sep = "\t", comment.char = "#")
#查看平台文件列名
colnames(platform_file1)

##数据处理----
# 假设芯片平台文件中有两列，一列是探针ID，一列是基因名
probe_names <- platform_file1$ID
gene_symbol <- platform_file1$Gene.Symbol

platform_file_set=platform_file1[,c(1,11)]
...
save(exprSet_clean, file ="exprSet_gse68465.RData")


#临床信息
patient1<-data.frame(ID=gse1@phenoData@data[["geo_accession"]],OS.time=gse1@phenoData@data[["months_to_last_contact_or_death:ch1"]],
                    OS=gse1@phenoData@data[["vital_status:ch1"]])

patient1$OS<-ifelse(patient1$OS == "Alive",0,1)
write.table(patient1,"GSE68465Patient.txt",sep = "\t",quote = F, row.names = T)


load("exprSet_gse68465.RData")
patient1<-read.table("GSE68465Patient.txt",
...
gse68465<-merge(patient1,tarexp1,by="ID")
...
gse68465_1<-gse68465[gse68465$ID %in% tumor_group$ID,]

write.table(gse68465_1,file = "gse68465_geneset.txt",sep = "\t",quote = F, row.names = T)
